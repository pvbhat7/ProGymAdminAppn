...
public class UserDaoImpl implements UserDao {
			...
			while(var6.hasNext()) {
				...
				if (!m.getDaysRemaining().equals("-")) {
					...
					if (filter.equalsIgnoreCase("sendToMinusDaysMembers")) {
						if (daysRemaining == 0) {
							if (!m.getPaymentStatus().equalsIgnoreCase("fully-paid")) {
								session.save(new SmsLogs(m.getName(), m.getMobile(), "Hi " + m.getName() + "\nYour package " + m.getPackageName() + " will expire today\nKindly pay remaining fees Rs." + m.getPendingFees() + " by clicking on this link mpay.me/8796655176 \n ProGym Kolhapur", this.getDdMmYyyyDate(""), "0", filter, m.getGender()));
							} else {
								session.save(new SmsLogs(m.getName(), m.getMobile(), "Hi " + m.getName() + "\nYour package " + m.getPackageName() + " will expire today\nKindly renew your package\n ProGym Kolhapur", this.getDdMmYyyyDate(""), "0", filter, m.getGender()));
							}
						}
						...
						if (daysRemaining < 0 && daysRemaining > -6) {
							session.save(new SmsLogs(m.getName(), m.getMobile(), "Hi " + m.getName() + "\nYour package " + m.getPackageName() + " expired " + Math.abs(Long.valueOf(m.getDaysRemaining())) + " days ago\nKindly renew your package\n ProGym Kolhapur", this.getDdMmYyyyDate(""), "0", filter, m.getGender()));
						}
					}
				}
			}
    ...
    public List<SmsLogs> getSmsByFilter(String gender) {
        ...
        crit.add(Restrictions.eq("timestamp", getDdMmYyyyDate("")));
